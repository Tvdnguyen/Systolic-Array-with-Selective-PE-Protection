`timescale 1ns / 1ps
// ============================================================
// pe_tmr.v  —  Triple Modular Redundancy PE Wrapper
//
// Instantiates 3 identical pe.v copies (pe_A, pe_B, pe_C).
// Majority voter (bitwise 2-of-3) selects the correct output.
// A single faulty PE is silently corrected — no error flag.
//
// Overhead vs plain pe: ~3x LUTs, ~3x DSPs, no net latency.
// ============================================================
module pe_tmr
#(
    parameter D_W = 8,
    ...
    // No error_flag: TMR silently corrects single faults
);
    ...
    pe #(.D_W(D_W),.i(i),.j(j)) pe_C (
        .clk(clk),.rst(rst),.init(init),.in_a(in_a),.in_b(in_b),
        .out_a(out_a_C),.out_b(out_b_C),
        .in_data(in_data),.in_valid(in_valid),
        .out_data(out_data_C),.out_valid(out_valid_C));

    // ── Bitwise 2-of-3 majority voter ────────────────────────
    assign out_data  = (out_data_A  & out_data_B)  |
                       (out_data_B  & out_data_C)  |
                       (out_data_A  & out_data_C);

    assign out_valid = (out_valid_A & out_valid_B) |
                       (out_valid_B & out_valid_C) |
                       (out_valid_A & out_valid_C);
