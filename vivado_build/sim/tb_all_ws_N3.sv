...
always @(posedge clk) begin
    for (br=0;br<N;br=br+1) begin
        m0_out[br] <= (rd_en_m0[br] && rd_addr_m0[br]<3) ? bram_m0[br][rd_addr_m0[br]] : 0;
        m1_out[br] <= (rd_en_m1[br] && rd_addr_m1[br]<3) ? bram_m1[br][rd_addr_m1[br]] : 0;
    end
end
wire [2*DW-1:0] m2 [N-1:0]; wire [N-1:0] valid_m2;
systolic_ws #(.D_W(DW),.N(N),.M(M)) dut(.clk(clk),.rst(rst),.enable_row_count_m0(enable_row_count_m0),.column_m0(column_m0),.row_m0(row_m0),.column_m1(column_m1),.row_m1(row_m1),.m0(m0_out),.m1(m1_out),.m2(m2),.valid_m2(valid_m2));
